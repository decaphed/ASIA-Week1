...
def test_mean_median_min_max_stddev_known_values():
    ...
    stats_window = window
    capped_by_metric = {m: (values if m == "vibration" else [2.0] * 4) for m in METRICS}

    result = aggregate_window(window, stats_window, capped_by_metric)
    vib = result["stats"]["vibration"]
...
def test_last_skips_trailing_null_metric_fill_row():
    window = [
        make_uniform_sample(timestamp="2026-01-01T00:00:00.000Z", vibration=2.0),
        make_uniform_sample(timestamp="2026-01-01T00:00:01.000Z", vibration=2.5),
        make_uniform_sample(timestamp="2026-01-01T00:00:02.000Z", vibration=None),  # trailing unfilled
    ]
    ...
    assert result["stats"]["vibration"]["last"] == 2.5  # skips the None raw value


def test_all_null_for_a_metric_raises():
    window = [make_uniform_sample(vibration=None)]
    capped_by_metric = {m: ([None] if m == "vibration" else [2.0]) for m in METRICS}
...
def test_population_variance_not_sample_variance():
    ...
    assert result["stats"]["vibration"]["stdDev"] == 4.33
